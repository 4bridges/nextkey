...
await page.waitForFunction(() => /donations found|Spenden/.test(document.getElementById('gifts')?.textContent ?? ''),
  null, { timeout: 15_000 })
const gifts = await page.textContent('#gifts')
check('the donations are counted', /2\s*stablecoin donations found/.test(gifts.replace(/\s+/g, ' ')))
...
check('what it cannot list is said, not hidden', /emits no event/.test(gifts))
