...
```bash
npm test                       # all eight suites, 333 checks
npm run test:crypto            # 60 — no bundle needed, and only interop wants a browser
npm run test:pages             # 273 — builds first, then drives the pages in Chromium
...
node web/test/blog.mjs         # 54 — the community page, its names, its editing step, and the
